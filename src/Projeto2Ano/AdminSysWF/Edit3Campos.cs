...
namespace AdminSysWF
{
    ...
    public partial class Edit3Campos : Form
    {
        ...
        public Edit3Campos(int id, Tabelas tabela, string label1, string label2, string label3, string placeholder1, string placeholder2, string placeholder3)
        {
            this.id = id;
            this.tabela = tabela;
            InitializeComponent();
        ...
        private void ConfirmEdit_Click(object sender, EventArgs e)
        {
            switch (tabela)
            {
                ...
                case Tabelas.Produto:
                    Database.EditProduto(id, txb_edit1.Text, int.Parse(txb_edit2.Text), int.Parse(txb_edit3.Text));
                    break;
            }
